...
const Sidebar = ({isOpen, Toggle}) => {
  ...
  const managerRouter = {
    ...
    User: { name: "User", displayIcon: userRoleId === "1" ? "none" : "inline-block", url: `${permission === "1" ? "/404" : "/admin/user"}`, display: userRoleId === "1" ? "none" : isOpen ? 'inline' : 'none', icon: faUser },
  };
  ...
  return (
    <SidebarCol style={{ width: isOpen ? "16%" : "6%" }} className={isOpen ? "col-sm-3 col-lg-2" : "col-sm-3 col-lg-2"}>
      ...
      <SidebarInner>
        <SidebarCategory style={{ display: userRoleId === "1" ? "none" : isOpen ? "inline-block" : "none" }}>Management</SidebarCategory>
